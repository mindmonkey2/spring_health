...
3. **Sub-Task 2: Member App - Wiring**
   ...
   - Use `replace_with_git_merge_diff` to update `spring_health_member_app/lib/screens/home/home_screen.dart`: Add a Goal Progress card above the session banner. Fetch the active goal and display progress without emojis.
...
5. **Run Analysis:**
   - Use `run_in_bash_session` to run `flutter analyze` on both apps to ensure exactly "No issues found!".
6. **Pre-commit Steps**
