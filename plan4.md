...
3. **Sub-Task 2: Member App - Wiring**
   ...
   - Use `replace_with_git_merge_diff` to update `spring_health_member_app/lib/screens/home/home_screen.dart`: Add a Goal Progress card. Since I read `home_screen.dart` previously, I know `_buildAiCoachBanner()` is the exact method representing the AI Personal Trainer banner (session banner). I will place my custom widget `_buildGoalProgressCard()` immediately above `_buildAiCoachBanner().animate().fadeIn(delay: 250.ms),` in the `build` method's `Column`. Fetch the active goal and display progress without emojis.
