...
3. **Sub-Task 2: Member App - Wiring**
   ...
   - Use `replace_with_git_merge_diff` to update `spring_health_member_app/lib/screens/home/home_screen.dart`: Add a Goal Progress card. I read `home_screen.dart` lines 320-350 previously and confirmed that `_buildAiCoachBanner()` is the session banner directly below `MembershipCardWidget()`. I will insert the new `_buildGoalProgressCard()` call immediately BEFORE the line `// ── AI Personal Trainer Banner ───────────────────────` within the `children` list. Fetch the active goal in `_HomeScreenState` and pass it to `_buildGoalProgressCard()`.
