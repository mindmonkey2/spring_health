...
3. **Sub-Task 2: Member App - Wiring**
   ...
   - Use `replace_with_git_merge_diff` to update `spring_health_member_app/lib/screens/home/home_screen.dart`: Add a Goal Progress card. I read `home_screen.dart` lines 340-360 previously and confirmed that `_buildAiCoachBanner()` is the session banner below `MembershipCardWidget()` on line 343. I will insert the new `_buildGoalProgressCard()` call immediately BEFORE the line `// ── AI Personal Trainer Banner ───────────────────────` (line 342) within the `children` list. Add a `_memberGoal` property to `_HomeScreenState`, update `_loadMemberData` to fetch the goal using `MemberGoalService`, and pass it to `_buildGoalProgressCard()`.
