...
class _TrainerReadinessScreenState extends State<TrainerReadinessScreen> {
  ...
  String _aiRecoveryStatus = 'No AI data yet';
    ...
    try {
      ...
      if (memberAuthUid.isNotEmpty) {
        final aiPlanDoc = await FirebaseFirestore.instance
            ...
            .get();
        if (aiPlanDoc.exists) {
          _aiRecoveryStatus = aiPlanDoc.data()?['recoveryStatus'] as String? ?? 'No AI data yet';
        }
      }
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('AI Recovery Status', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
            const SizedBox(height: 12),
            Chip(
