...
enum WorkoutType {
  ...
  String get label {
    switch (this) {
      case WorkoutType.upperBody: return 'Upper Body Power';
      case WorkoutType.cardio:    return 'Cardio Blast';
      case WorkoutType.yoga:      return 'Yoga Flow';
      case WorkoutType.legDay:    return 'Leg Day';
      case WorkoutType.fullBody:  return 'Full Body';
      case WorkoutType.hiit:      return 'HIIT';
      case WorkoutType.other:     return 'Workout';
    }
  }

  String get emoji {
    switch (this) {
      case WorkoutType.upperBody: return '💪';
      case WorkoutType.cardio:    return '🏃';
      case WorkoutType.yoga:      return '🧘';
      case WorkoutType.legDay:    return '🦵';
      case WorkoutType.fullBody:  return '🏋️';
      case WorkoutType.hiit:      return '⚡';
      case WorkoutType.other:     return '🏅';
    }
  }

  static WorkoutType fromString(String raw) {
    final s = raw.toLowerCase();
    if (s.contains('upper') || s.contains('power')) return WorkoutType.upperBody;
...
class FitnessStats {
  ...
  double stepProgress({int goal = 8000}) =>
      (steps / goal).clamp(0.0, 1.0);

  /// Progress toward daily calorie goal (0.0 – 1.0)
  double calorieProgress({int goal = 500}) =>
      (calories / goal).clamp(0.0, 1.0);

  /// Formatted distance string
  String get distanceLabel =>
      distance >= 1.0 ? '${distance.toStringAsFixed(1)} km' : '${(distance * 1000).toInt()} m';
  ...
  int get hashCode => Object.hash(
    steps, calories, distance, heartRate, maxHeartRate,
    activeMinutes, date.year, date.month, date.day,
  );
...
class WorkoutSession {
  ...
  static WorkoutSource _sourceFromString(String s) {
    switch (s) {
      case 'healthKit':     return WorkoutSource.healthKit;
      case 'healthConnect': return WorkoutSource.healthConnect;
      default:              return WorkoutSource.manual;
    }
  }

  String get sourceLabel {
    switch (source) {
      case WorkoutSource.healthKit:     return 'Apple Health';
      case WorkoutSource.healthConnect: return 'Health Connect';
      case WorkoutSource.manual:        return 'Manual';
    }
  }
