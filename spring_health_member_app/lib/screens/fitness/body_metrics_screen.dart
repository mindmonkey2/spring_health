...
class BodyMetricsScreen extends StatefulWidget {
  ...
  const BodyMetricsScreen({super.key, required this.memberId});
...
with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<List<BodyMetricsModel>>(
        ...
        builder: (context, snapshot) {
          ...
          if (metrics.isEmpty) return _buildEmpty();
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // ✅ FIXED
                ...metrics.asMap().entries.map((e) => _buildCard(
                  e.value, e.key, metrics)
                .animate()
      ...
      floatingActionButton: StreamBuilder<List<BodyMetricsModel>>(
        stream: _service.getMetricsStream(widget.memberId),
        builder: (context, snap) {
          final list = snap.data ?? [];
          return FloatingActionButton.extended(
            onPressed: () => _openSheet(list),
            backgroundColor: AppColors.neonLime,
            foregroundColor: Colors.black,
              icon: const Icon(Icons.add_rounded),
              label: Text('LOG METRICS',
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(DateFormat('dd MMM yyyy').format(m.recordedAt),
          ...
          Row(
            ...
            children: [
              ...
              AppColors.neonOrange, Icons.person_rounded),
            ],
          ),
      ...
      Text(value,
           ...
           Text(unit,
                style: AppTextStyles.caption.copyWith(
                  ...
                  Text(label,
                       style: AppTextStyles.caption
                       .copyWith(color: AppColors.gray400, fontSize: 9, letterSpacing: 1.5)),
    ]);
                   }
                                        ...
                                        return Center(
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              const SizedBox(height: 8),
                                              Text('Start tracking to see your progress over time',
                                                   style: AppTextStyles.bodyMedium
                                                   .copyWith(color: AppColors.gray400),
                                                   textAlign: TextAlign.center),
...
extends State<AddMetricsBottomSheet> {
  ...
  void dispose() {
    for (final c in [
      _weightCtrl, _heightCtrl, _bodyFatCtrl, _chestCtrl,
      _waistCtrl, _hipsCtrl, _armsCtrl, _thighsCtrl, _notesCtrl
    ]) {
      c.dispose();
    }
    super.dispose();
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 12),
                      _field(_weightCtrl, 'Weight *', 'e.g. 70.5',
                             'kg', Icons.monitor_weight_rounded,
                             AppColors.neonLime,
                             required: true),
                             const SizedBox(height: 12),
                             _field(_heightCtrl, 'Height (for BMI)',
                             'e.g. 170.0', 'cm', Icons.height_rounded,
                             AppColors.neonTeal),
                             const SizedBox(height: 12),
                             _field(_bodyFatCtrl, 'Body Fat',
                                    'e.g. 18.5', '%', Icons.person_rounded,
                                    AppColors.neonOrange),
                                const SizedBox(height: 20),
                                ...
                                if (_showMeasurements) ...[
                                  const SizedBox(height: 12),
                                  _field(_chestCtrl, 'Chest', 'e.g. 95.0', 'cm',
                                         Icons.accessibility_new_rounded,
                                         AppColors.neonTeal),
                                const SizedBox(height: 12),
                                _field(_waistCtrl, 'Waist', 'e.g. 80.0', 'cm',
                                       Icons.accessibility_new_rounded,
                                       AppColors.neonTeal),
                                const SizedBox(height: 12),
                                _field(_hipsCtrl, 'Hips', 'e.g. 95.0', 'cm',
                                       Icons.accessibility_new_rounded,
                                       AppColors.neonTeal),
                                const SizedBox(height: 12),
                                Row(
                                  children: [
                                    Expanded(
                                      child: _field(_armsCtrl, 'Arms',
                                                    'e.g. 35.0', 'cm',
                                                    Icons.fitness_center_rounded,
                                                    AppColors.neonTeal)),
                                                    const SizedBox(width: 12),
                                                    Expanded(
                                                      child: _field(_thighsCtrl, 'Thighs',
                                                                    'e.g. 55.0', 'cm',
                                                                    Icons.fitness_center_rounded,
                                                                    AppColors.neonTeal)),
                                  ],
