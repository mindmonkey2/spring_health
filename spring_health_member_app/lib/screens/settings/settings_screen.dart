...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _sub(String topic)   => FirebaseMessaging.instance.subscribeToTopic(topic);
  Future<void> _unsub(String topic) => FirebaseMessaging.instance.unsubscribeFromTopic(topic);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionHeader('NOTIFICATIONS', Icons.notifications_rounded, AppColors.neonLime),
            const SizedBox(height: 12),
            _buildToggle('Push Notifications', 'Receive all app notifications',
                         Icons.notifications_active_rounded, AppColors.neonLime,
                         pushNotifications, (v) { if (v != null) _handlePushToggle(v); }, isMain: true),
                         const SizedBox(height: 8),
                         _buildToggle('Announcements', 'Gym updates and news',
                                      Icons.campaign_rounded, AppColors.neonTeal,
                                      announcementAlerts && pushNotifications,
                                      (v) { if (v != null && pushNotifications) _handleAnnouncementToggle(v); },
                                      disabled: !pushNotifications),
                      const SizedBox(height: 8),
                      _buildToggle('Expiry Reminders', '7 days before membership expires',
                                   Icons.event_rounded, AppColors.neonOrange,
                                   expiryReminders && pushNotifications,
                                   (v) { if (v != null && pushNotifications) _handleExpiryToggle(v); },
                                   disabled: !pushNotifications),
                      const SizedBox(height: 8),
                      _buildToggle('Check-in Confirmations', 'Notify when QR is scanned',
                                   Icons.qr_code_scanner_rounded, AppColors.turquoise,
                                   checkInConfirmations && pushNotifications,
                                   (v) { if (v != null && pushNotifications) _handleCheckinToggle(v); },
                                   disabled: !pushNotifications),
                      const SizedBox(height: 8),
                      _buildToggle('Offers & Promotions', 'Special deals and discounts',
                                   Icons.local_offer_rounded, AppColors.gray400,
                                   ...
                                   disabled: !pushNotifications),
                      const SizedBox(height: 24),
                      _buildSectionHeader('APP PREFERENCES', Icons.tune_rounded, AppColors.neonTeal),
                      const SizedBox(height: 12),
                      _buildToggle('Haptic Feedback', 'Vibration on button taps',
                                   Icons.vibration_rounded, AppColors.neonTeal,
                                   hapticFeedback,
                                   (v) {
                                     if (v != null) {
                      ...
                      const SizedBox(height: 8),
                      _buildToggle('Biometric Lock', 'Use fingerprint to open app',
                                   Icons.fingerprint_rounded, AppColors.neonLime,
                                   biometricLock,
                                   (v) { if (v != null) setState(() => biometricLock = v); }),
                                   const SizedBox(height: 24),
                                   _buildSectionHeader('ACCOUNT', Icons.manage_accounts_rounded, AppColors.neonOrange),
                                   const SizedBox(height: 12),
                                   _buildActionTile('Member ID', widget.member.id,
                                                    Icons.badge_rounded, AppColors.neonLime,
                                                    onTap: () {
                                                      Clipboard.setData(ClipboardData(text: widget.member.id));
                                                      _showSnack('Member ID copied!');
                                                    },
                                                    trailing: const Icon(Icons.copy_rounded, size: 16, color: AppColors.gray400)),
                                                    const SizedBox(height: 8),
                                                    _buildActionTile('Phone', widget.member.phone,
                                                                     Icons.phone_rounded, AppColors.neonTeal),
                      const SizedBox(height: 8),
                      _buildActionTile('Branch', widget.member.branch.toUpperCase(),
                      Icons.location_on_rounded, AppColors.neonOrange),
                      const SizedBox(height: 24),
                      _buildSectionHeader('ABOUT', Icons.info_rounded, AppColors.turquoise),
                      const SizedBox(height: 12),
                      _buildActionTile('App Version', '1.0.0',
                                       Icons.system_update_rounded, AppColors.turquoise),
                      const SizedBox(height: 8),
                      _buildActionTile('Privacy Policy', 'View our privacy terms',
                                       Icons.privacy_tip_rounded, AppColors.gray400,
                                       ...
                                       const SizedBox(height: 8),
                                       _buildActionTile('Terms of Service', 'Read terms and conditions',
                                                        Icons.description_rounded, AppColors.gray400,
                                                        onTap: () => _showSnack('Terms of Service coming soon!'),
                                                        trailing: const Icon(Icons.chevron_right_rounded, size: 20, color: AppColors.gray400)),
                                                        const SizedBox(height: 8),
                                                        _buildActionTile('Contact Support', 'Get help from our team',
                                                                         Icons.headset_mic_outlined, AppColors.neonTeal,
                                                                         onTap: () => _showContactSupport(context),
                                                                         trailing: const Icon(Icons.chevron_right_rounded, size: 20, color: AppColors.gray400)),
                                                                         const SizedBox(height: 24),
                                                                         _buildSectionHeader('DANGER ZONE', Icons.warning_rounded, AppColors.error),
                                                                         const SizedBox(height: 12),
                                                                         _buildDangerButton('Clear App Cache', Icons.cleaning_services_rounded,
                                                                                            AppColors.neonOrange, _handleClearCache),
                      const SizedBox(height: 8),
                      _buildDangerButton('Logout', Icons.logout_rounded,
                                         AppColors.error, _handleLogout),
        ...
        Container(
          ...
          child: Center(
            child: Text(
              ...
              .copyWith(color: AppColors.neonLime, fontSize: 24),
            ),
          ),
        ),
          ...
          child: Column(
            ...
            children: [
              Text(widget.member.name,
                   ...
                   const SizedBox(height: 4),
                   Text(widget.member.membershipPlan,
  ...
  Widget _buildToggle(
    ...
    bool value, ValueChanged<bool?> onChanged, {
      bool isMain = false, bool disabled = false,
    }) =>
    AnimatedOpacity(
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(icon,
                          color: value ? color : AppColors.gray600, size: 20),
            ),
            const SizedBox(width: 14),
            Expanded(
              child: Column(
        ...
        void _handleClearCache() {
          showDialog(
            ...
            builder: (_) => AlertDialog(
              ...
              content: Text('This will clear temporary files. Your data is safe.',
            ...
            builder: (_) => Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Icon(Icons.headset_mic_outlined, color: AppColors.neonTeal, size: 40),
                  const SizedBox(height: 12),
                  Text('Contact Support',
                       style: AppTextStyles.heading3.copyWith(color: AppColors.neonTeal)),
                       const SizedBox(height: 24),
                       _contactRow(Icons.location_on_rounded, 'Hanamkonda & Warangal Branches'),
                       const SizedBox(height: 14),
                       _contactRow(Icons.access_time_rounded, 'Mon-Sat: 6:00 AM – 10:00 PM'),
                       const SizedBox(height: 14),
