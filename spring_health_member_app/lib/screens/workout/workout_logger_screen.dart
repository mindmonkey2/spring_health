...
with SingleTickerProviderStateMixin { // ✅ for live timer
  ...
  TextEditingController(text: 'Morning Workout');
  ...
  final Map<String, List<String>> _exerciseLibrary = {
    'Chest': [
      ...
      'Push Ups', 'Cable Flyes', 'Dumbbell Flyes', 'Chest Dips',
    ],
    'Back': [
      'Pull Ups', 'Lat Pulldown', 'Seated Cable Row', 'Bent Over Row',
      'Deadlift', 'T-Bar Row', 'Single Arm Row',
    ],
    'Legs': [
      'Squats', 'Leg Press', 'Romanian Deadlift', 'Leg Curl',
      'Leg Extension', 'Calf Raises', 'Lunges', 'Hack Squat',
    ],
    'Shoulders': [
      'Overhead Press', 'Lateral Raises', 'Front Raises',
      'Rear Delt Flyes', 'Arnold Press', 'Upright Row', 'Shrugs',
    ],
    'Arms': [
    ...
    'Core': [
      ...
      'Ab Wheel', 'Cable Crunch', 'Side Plank', 'Mountain Climbers',
    ],
    'Cardio': [
      'Treadmill', 'Cycling', 'Jump Rope', 'Rowing Machine',
      'Stair Climber', 'Elliptical', 'HIIT Sprints',
    ],
  };
  ...
  int get _totalSets =>
  _exercises.fold(0, (total, e) => total + e.sets.length);
  ...
  TextEditingController _getWeightController(
    String exerciseId, int setIndex, double currentWeight) {
    ...
    TextEditingController _getRepsController(
      String exerciseId, int setIndex, int currentReps) {
      ...
      void _removeSetControllers(String exerciseId, int setIndex) {
        _weightControllers
        .remove(_weightKey(exerciseId, setIndex))
        ?.dispose();
        _repsControllers
        .remove(_repsKey(exerciseId, setIndex))
        ?.dispose();
      }
      ...
      Widget _buildStatChip(
          ...
          return AnimatedContainer(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle:
                AppTextStyles.caption.copyWith(color: AppColors.gray600),
                filled: true,
        ...
        Widget _buildSetRow(int exerciseIndex, int setIndex) {
          ...
          final weightController = _getWeightController(
            exercise.id, setIndex, set.weight);
          final repsController =
          _getRepsController(exercise.id, setIndex, set.reps);
        ...
        Widget _buildNumberInput(
            ...
            showModalBottomSheet(
              ...
              builder: (context) => StatefulBuilder(
                builder: (context, setModalState) {
                  final exercises =
                  (_exerciseLibrary[selectedCategory] ?? [])
                  .where((e) => e
                  .toLowerCase()
          ...
          // ─────────────────────────────────────
          void _showWorkoutSummary(WorkoutLog workout,
                        ...
                        .map(
                          (b) => Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              shape: BoxShape.circle,
                              border:
                              Border.all(color: b.color, width: 1.5),
                            ),
                                     ...
                                     void _removeSet(int exerciseIndex, int setIndex) {
                                       if (_exercises[exerciseIndex].sets.length <= 1) {
                                         ScaffoldMessenger.of(context).showSnackBar(
                                           const SnackBar(
                                             content: Text(
                                               'At least 1 set required. Delete exercise instead.'),
