...
class BadgeService {
  ...
  static const List<Map<String, dynamic>> badgeDefinitions = [
    ...
    {'id': 'war_win',         'label': 'War Champion',      'type': 'war_wins',  'threshold': 1},
    {'id': 'loyalty_3m',      'label': 'Loyal Member',      'type': 'loyalty_months', 'threshold': 3},
    {'id': 'loyalty_1y',      'label': 'Spring Legend',     'type': 'loyalty_months', 'threshold': 12},
  ];

  Future<void> checkAndAward(String memberId) async {
    final doc = await FirebaseFirestore.instance
        .collection('gamification').doc(memberId).get();
    ...
    for (final badge in badgeDefinitions) {
      ...
      bool earned = false;
      switch (badge['type']) {
        case 'xp':       earned = currentXP >= badge['threshold']; break;
        case 'streak':   earned = currentStreak >= badge['threshold']; break;
        case 'workouts': earned = workoutCount >= badge['threshold']; break;
        case 'checkins': earned = checkinCount >= badge['threshold']; break;
        case 'pbs':      earned = pbCount >= badge['threshold']; break;
        case 'war_wins': earned = warWins >= badge['threshold']; break;
        case 'loyalty_months': earned = loyaltyMonths >= badge['threshold']; break;
      }
    ...
    await FirebaseFirestore.instance.collection('gamification').doc(memberId)
