...
class ChallengeService {
  ...
  : ChallengeModel.fromFirestore(
    s.docs.first.data(), s.docs.first.id));
  ...
  Future<void> logProgress({
    ...
    try {
      ...
      final challengeRef = _db.collection('challenges').doc(challengeId);
      final scoreField =
      teamId == 'teamA' ? 'teamA.totalScore' : 'teamB.totalScore';
      batch.update(challengeRef, {scoreField: FieldValue.increment(diff)});

      await batch.commit();
      debugPrint('ChallengeService: Score updated — new: $newScore  diff: $diff');
    } catch (e) {
  ...
  Future<void> createDemoChallenge() async {
    ...
    await _db.collection('challenges').add(ChallengeModel(
      id:          '',
      ...
      teamA: const ChallengeTeam(
        id: 'teamA', name: 'Alpha Squad', emoji: '🔥',
        totalScore: 0, memberIds: [],
      ),
      teamB: const ChallengeTeam(
        id: 'teamB', name: 'Beast Mode', emoji: '⚡',
        totalScore: 0, memberIds: [],
      ),
      prizeXP:     500,
      description: 'Walk your way to victory! Most steps at end of the week wins 500 XP each.',
      createdAt:   now,
    ).toMap());
  }
}
