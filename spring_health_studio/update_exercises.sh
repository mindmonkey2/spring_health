...
<<<<<<< SEARCH
              const Expanded(
                child: Center(child: Text('Exercises List coming soon.')),
              ),
            ],
          );
        },
...
=======
  ...
  Widget _buildEndSessionCard() {
    return const Center(child: Text('End Session UI coming soon'));
  }
}
INNER_EOF
patch lib/screens/trainer/trainer_session_screen.dart merge.patch
